...
// document.addEventListener('DOMContentLoaded', function() {
  ...
  async function fetchCategories() {
    try {
      ...
      categories = fetchedCategories.length > 0 ? fetchedCategories : [
        { name: 'Income', subcategories: ['Salary', 'Freelance', 'Investments'] },
        { name: 'Expense', subcategories: ['Rent', 'Groceries', 'Utilities'] },
        { name: 'Transfer', subcategories: ['Bank Transfer', 'Cash Withdrawal'] },
        { name: 'Asset', subcategories: ['Real Estate', 'Stocks', 'Mutual Fund'] },
        { name: 'Liability', subcategories: ['Loan', 'Credit Card Debt', 'Home Loan Amount'] },
        { name: 'Goals', subcategories: ['Vacation', 'Car', 'House'] }
      ];
  ...
  function populateCategoryDropdown() {
    ...
    if (categorySelect && saveTransactionBtn) {
      categorySelect.addEventListener('change', function() {
        saveTransactionBtn.disabled = !validCategories.includes(this.value);
  ...
  document.getElementById('saveNewCategoryBtn').addEventListener('click', async function(e) {
    ...
    const subcategories = document.getElementById('newSubcategories').value.split(',').map(s => s.trim());
    ...
    try {
      ...
      if (!response.ok) throw new Error(`Category save failed: HTTP ${response.status}`);
  ...
  if (saveTransactionBtn) {
    saveTransactionBtn.addEventListener('click', async function(e) {
      e.preventDefault();
      const transactionData = {
        ...
        subcategory: document.getElementById('transactionSubcategory')?.value || null,
      ...
      try {
        ...
        if (!response.ok) throw new Error(`Transaction save failed: HTTP ${response.status}`);
        const data = await response.json();
        alert(data.message || 'Transaction saved successfully!');
        // fetchAndRenderDashboardCharts();
        await fetchTransactions();
       
        await fetchSectionData(transactionData.category.toLowerCase());
